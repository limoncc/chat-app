...
// ObjC 侧 target：接收两个控件的 action，映射到站点切换 / 打开设置。
define_class!(
...
pub fn rebuild(window: &tauri::Window) -> tauri::Result<()> {
    ...
    cycle_btn.setFrameSize(NSSize::new(cycle_w, CONTROL_H));
    settings_btn.setFrameSize(NSSize::new(settings_w, CONTROL_H));
